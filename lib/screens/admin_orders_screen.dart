...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
  ...
  Future<void> _updateOrderStatus(String orderId, String newStatus) async {
    try {
      final orderStatus = OrderStatus.values.firstWhere(
        (e) => e.toString() == 'OrderStatus.' + newStatus,
        orElse: () => OrderStatus.pending, // Default to pending if not found
      );
      await _dataRepository.updateOrderStatus(orderId, orderStatus);
      setState(() {
  ...
  Future<void> _launchWhatsApp(String mobileNumber) async {
    ...
    if (await canLaunchUrl(launchUri)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Could not launch WhatsApp. Ensure it is installed.'),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          _isLoading &&
                  _orders
                      .isEmpty // Show loading indicator only if no orders are loaded yet
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Total Amount: KES ${order['totalAmount']?.toStringAsFixed(2) ?? 'N/A'}',
                          ),
                          Row(
                            children: [
                              const Text('Status: '),
                          ...
                          Text(
                            'Order Date: ${order['timestamp'] != null ? (order['timestamp'].toDate()).toLocal().toString().split('.')[0] : 'N/A'}',
                          ),
                          Row(
